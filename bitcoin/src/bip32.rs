...
use crate::internal_macros::{impl_array_newtype, impl_bytes_newtype, write_err};
...
impl Xpriv {
    ...
    pub fn new_master(network: impl Into<NetworkKind>, seed: &[u8]) -> Result<Xpriv, Error> {
        ...
        sk_bytes.copy_from_slice(&hmac_result[..32]);
        ...
        Ok(Xpriv {
            ...
            private_key: secp256k1::SecretKey::from_secret_bytes(sk_bytes)?,
    ...
    fn ckd_priv<C: secp256k1::Signing>(
        ...
        let mut sk_bytes = [0u8; 32];
        sk_bytes.copy_from_slice(&hmac_result[..32]);
        let sk = secp256k1::SecretKey::from_secret_bytes(sk_bytes)
    ...
    pub fn decode(data: &[u8]) -> Result<Xpriv, Error> {
        ...
        let mut sk_bytes = [0u8; 32];
        sk_bytes.copy_from_slice(&data[46..78]);
        Ok(Xpriv {
            network,
            depth: data[4],
    ...
    pub fn ckd_pub_tweak(
        ...
        match i {
            ...
            ChildNumber::Normal { index: n } => {
                ...
                let mut sk_bytes = [0u8; 32];
                sk_bytes.copy_from_slice(&hmac_result[..32]);
                let private_key = secp256k1::SecretKey::from_secret_bytes(sk_bytes)?;
